...
export const HarborTrade =
  ({ parameters }: HarborTradeAction) =>
  (w: Result): Result =>
    w
      ...
      .flatMap(transferResources(parameters.playerName, parameters.transfer, parameters.receive));
...
const resourcesMatchHarbor =
  (check: Resources, harborType: HarborType) =>
  (w: World): Result => {
    ...
    switch (harborType) {
      ...
      case 'ThreeToOneHarbor':
        return check.clay >= 3 || check.grain >= 3 || check.stone >= 3 || check.wood >= 3 || check.wool >= 3
          ? succeeded
          : failed;
      default:
...
const playerHasHarbor =
  (harborType: HarborType) =>
  (w: World): Result => {
    const player = w.players[w.currentPlayer]!;
    const tiles = w.map;
    const hasHarbor = player.houses.some((house) =>
      neighbouringHexCoords(house.position).some((h) =>
        tiles.some((t) => t.coord.x === h.x && t.coord.y === h.y && t.type === harborType),
      ),
    );
    if (!hasHarbor) {
