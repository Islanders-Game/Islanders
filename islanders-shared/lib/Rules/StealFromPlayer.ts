import { type Result, type TileType, World, fail, success } from '../Shared';
import { StealFromPlayerAction } from '../Action';
import { ensureGameState, findPlayer, getResourceAmountOfType, setAmountOfResourceOfType } from './Helpers';

export const StealFromPlayer =
  ({ parameters }: StealFromPlayerAction) =>
  (world: Result): Result =>
    world
...
const stealFromPlayer =
  ...
  (w: World): Result => {
    ...
    const newChosenPlayer = {
      ...chosenPlayer,
      resources: setAmountOfResourceOfType(newResourceCountForChosenPlayer, chosenPlayer.resources, chosenResourceType),
    };
    const newCurrentPlayer = {
      ...currentPlayer,
      resources: setAmountOfResourceOfType(
        newResourceCountForCurrentPlayer,
