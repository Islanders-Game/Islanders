...
import { fail, type Result, success, World } from '../Shared';
import { findPlayer, hasResources, playerExists, transferResources } from './Helpers';
import { empty, type Resources, resourcesAreNonNegative } from '../Resources';

export const PlayerTrade =
  ({ parameters }: PlayerTradeAction) =>
  (w: Result): Result =>
    w
      ...
      .flatMap(transferResources(parameters.playerName, empty, parameters.receivedResources))
      .flatMap(transferResources(parameters.otherPlayerName, parameters.sentResources, empty));
