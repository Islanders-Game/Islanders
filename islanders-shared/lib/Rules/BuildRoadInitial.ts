...
import { type Result, World, fail } from '../Shared';
...
const checkNumberOfStructures = (w: World): Result => {
  ...
  if (currentPlayer!.roads.length > round) {
    return fail(
      `You cannot place a ${ordinal(currentPlayer!.roads.length + 1)} road in the ${ordinal(round + 1)} pre-game round`,
    );
  }
  return success({ ...w, conditions: { ...w.conditions, mustPlaceInitialRoad: { hasPlaced: true } } });
};

export const BuildRoadInitial =
  ({ parameters }: BuildRoadInitialAction) =>
  (world: Result): Result =>
    world
