...
import { type Result, type TileType, success, fail, World } from '../Shared';
import {
  ensureGameState,
  findPlayer,
  getResourceAmountOfType,
  increasePointsForPlayer,
  setAmountOfResourceOfType,
} from './Helpers';

export const PlayCard =
  ({ parameters }: PlayCardAction) =>
  (w: Result): Result =>
    w
      .flatMap(ensureGameState('Started'))
      .flatMap(findPlayer(parameters.playerName))
      .flatMap(playCard(parameters.playerName, parameters.card, parameters.chosenResources));

const playCard =
  (playerName: string, card: DevelopmentCard, chosenResources?: [TileType] | [TileType, TileType]) =>
  (w: World): Result => {
    ...
    if (card.type === 'Knight') {
      ...
      return success({
        ...w,
        players,
        conditions: { ...w.conditions, playedKnight: { movedThief: false, stoleFromPlayer: false } },
      });
    }
    if (card.type === 'Road Building') {
      ...
      const intermediate = success({
        ...
        conditions: {
          ...
          playedRoadBuilding: {
            roadsBuilt: 0,
            expected: w.conditions.playedRoadBuilding ? w.conditions.playedRoadBuilding.expected + 2 : 2,
          },
        },
      });
      return intermediate;
    ...
    if (card.type === 'Monopoly') {
      ...
      const toReceive = allOthersResources.reduce((acc, rr) => acc + getResourceAmountOfType(chosenType[0], rr), 0);
      ...
      const players = w.players.map((pl) =>
        pl.name === playerName
          ...
          : {
              ...pl,
              resources: setAmountOfResourceOfType(0, pl.resources, chosenType[0]),
            },
      );
      return success({
        ...w,
