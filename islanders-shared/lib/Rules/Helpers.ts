...
import type { Tile, DiceRoll, TileType } from '../Tile';
import { type Result, fail, success } from './Result';
import { World, type GameState } from '../World';
import { Player } from '../Player';
import { addResources } from '../Resources';
import type { TileRessource } from './TileRessource';
...
export const increasePointsForPlayer =
  (name: string) =>
  (w: World): Result => {
...
export const purchase =
  ...
  (w: World): Result => {
    ...
    const resources = w.conditions.playedRoadBuilding ? player.resources : subtractResources(player.resources, cost);
...
export const hasResources =
  (playerName: string, check: Resources) =>
  (w: World): Result => {
...
export const transferResources =
  (playerName: string, remove: Resources, assign: Resources) =>
  (w: World): Result => {
    const player = w.players[w.currentPlayer];
...
export const placeHouse =
  ...
  (playerName: string) =>
  (hasRoad: (coordinate: HexCoordinate, p: Player) => boolean) =>
  (world: World): Result => {
    ...
    const concatHouseIfMatch = (pl: Player) =>
      pl.name === playerName
        ? {
            ...pl,
            houses: pl.houses.concat([new House(coord)]),
          }
        : pl;
...
export const placeRoad =
  (start: MatrixCoordinate, end: MatrixCoordinate) =>
  (playerName: string) =>
  (w: World): Result => {
    const player = w.players.find((pl) => pl.name === playerName);
    if (!player) return fail(`The player ${playerName} was not found`);
    ...
    const houseAtOneEnd = player.houses.some(
      (h) =>
        (h.position.x === start.x && h.position.y === start.y) || (h.position.x === end.x && h.position.y === end.y),
    );
...
export const moveThiefUsingKnight =
...
export const moveThiefUsingDiceRoll =
  (coords: HexCoordinate) =>
  (w: World): Result =>
