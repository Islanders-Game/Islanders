...
describe('Rules when playing development cards', () => {
  test('A player gaining a Knight development card (giving +1 knight) increases their number of knights by 1', () => {
    ...
    while (!stop) {
      ...
      if (intermediate.hasOwnProperty('value')) {
        const shouldStop = (intermediate as Success).value.players
          .find((p) => p.name === p1.name)!
          .devCards.some((c) => c.type === 'Knight');
        if (shouldStop) {
          stop = shouldStop;
          toTest = intermediate;
        }
  ...
  test('A player playing an already played development card they have is not allowed', () => {
    const p1: Player = new Player('P1');
    const devCard: DevelopmentCard = {
      cost: { grain: 1, stone: 1, wool: 1, wood: 0, clay: 0 },
      played: true,
      type: 'Road Building',
    } as DevelopmentCard;
    p1.devCards = [devCard];
