...
describe('Neighbouring HexCoordinates are found from MatrixCoordinates as expected', () => {
  ...
  const expectedHexesEven: HexCoordinate[] = [
    { x: 2, y: 0 },
    { x: 2, y: -1 },
    { x: 3, y: -1 },
  ];
  ...
  const expectedHexesOdd: HexCoordinate[] = [
    { x: 2, y: -1 },
    { x: 3, y: -2 },
    { x: 3, y: -1 },
  ];
  ...
  expect(
    evenConverted.every((converted) =>
      expectedHexesEven.some((expected) => expected.x === converted.x && expected.y === converted.y),
    ),
  );
  expect(
    oddConverted.every((converted) =>
      expectedHexesOdd.some((expected) => expected.x === converted.x && expected.y === converted.y),
    ),
  );
});
